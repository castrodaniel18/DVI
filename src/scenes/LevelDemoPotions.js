...
export default class LevelDemoPotions extends Phaser.Scene {
	constructor() {
		super({ key: 'LevelDemoPotions' });
		this.enemies = [];
		this.playerItems = [];
	...
	create() {
		...
		this.pauseButton.on('pointerdown', () => {
			//this.waveController.initialPauseTime = Date();
			//this.waveController.pause = true;
			this.scene.pause();
			this.scene.run('PauseScene', { difficulty: this.difficulty, actualScene: 'LevelDemoPotions' });
		});
	...
	update() {
		...
		if (this.player.isDead) {
			this.scene.pause();
			this.scene.run('GameOver', { difficulty: this.difficulty, characterName: this.characterName, level: 'LevelDemoPotions' })
		}
